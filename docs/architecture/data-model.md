...
| overall_confidence | VARCHAR(10) | Nullable — derived from criterion confidence levels: `low` if any criterion is `low`; `medium` if any is `medium` (and none is `low`); `high` if all are `high`. NULL for grades produced before M4.1. |
| created_at | TIMESTAMPTZ | |
