...
### StudentGroup
Auto-computed skill-gap group for a class, generated by the `compute_class_groups` Celery task (M6-01) after each grade lock. One row per (teacher_id, class_id, skill_key) — the unique constraint means re-running the task is fully idempotent.
